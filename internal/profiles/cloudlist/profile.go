...
func Profiles(maxLines int) []engine.Profile {
	out := awsProfiles(maxLines)
	return append(out, vercelDeploymentsProfile(maxLines), cloudListProfile(maxLines))
}

var vercelDeploymentsExplain = []string{
...
func matchVercelDeployments(inv engine.Invocation) bool {
	return len(inv.Display) > 0 && inv.Display[0] == "vercel" && isVercelDeploymentCommand(inv.Display)
}

func vercelDeploymentsProfile(maxLines int) engine.Profile {
	return engine.Profile{
		...
		Match:            matchVercelDeployments,
		Prepare: func(inv engine.Invocation) []string {
			return prepareVercelDeploymentCommand(inv.Command)
		},
		Render:       renderCloudList(maxLines),
		StreamRender: streamRenderCloudList,
		ParseBytes:   profilekit.ParseStdout,
		Explain:      vercelDeploymentsExplain,
	}
}

var cloudListExplain = []string{
...
func matchCloudList(inv engine.Invocation) bool {
	return isCloudListCommand(inv.Display)
}

func cloudListProfile(maxLines int) engine.Profile {
	return engine.Profile{
		Name:             "cloud-list",
		...
		StreamRender: streamRenderCloudList,
		ParseBytes:   profilekit.ParseStdout,
		Explain:      cloudListExplain,
	}
}

func renderCloudList(maxLines int) func(engine.Invocation, engine.Execution) string {
	return func(_ engine.Invocation, exec engine.Execution) string {
		return cloudfilter.SummarizeCloudList(exec.Stdout, maxLines)
	}
}

func streamRenderCloudList(_ engine.Invocation, budget engine.OutputBudget) engine.StreamReducer {
