...
func TestGitSummaries(t *testing.T) {
	...
	t.Run("success-paths", testGitSuccessPathSummary)
...
func testGitSuccessPathSummary(t *testing.T) {
	...
	if got := gitfilter.SummarizeGitAdd(addInv, ""); got != "staged internal/... (1), test/... (1)" {
		t.Fatalf("unexpected git add summary: %q", got)
	}
	addAllInv := engine.Classify(engine.Invocation{Command: []string{"git", "add", "-A"}})
	if got := gitfilter.SummarizeGitAdd(addAllInv, ""); got != "staged all changes" {
	...
	if got := gitfilter.SummarizeGitAdd(displayOnlyAdd, ""); got != "staged README.md" {
	...
	if got := gitfilter.SummarizeGitCommit(commitInv, "[main abc1234] tighten reducer\n 2 files changed, 7 insertions(+), 1 deletion(-)\n"); got != "committed abc1234 tighten reducer files=2 +7 -1" {
...
func assertGitSuccessReducers(t *testing.T) {
	t.Helper()

	pushReducer := gitfilter.NewGitSuccessPathReducer("push", engine.Invocation{}, 6, 0)
	...
	commitReducer := gitfilter.NewGitSuccessPathReducer("commit", engine.Invocation{}, 6, 0)
	...
	fallbackReducer := gitfilter.NewGitSuccessPathReducer("pull", engine.Invocation{}, 6, 0)
