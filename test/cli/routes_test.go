...
func TestRunRoutes(t *testing.T) {
	...
	cases := []struct {
		...
		{"commands", []string{"commands"}, 0, []string{"commands", "test", "Execution:", "Local Tools:", "Install:", "szr rg <pattern> [path]", "szr uninstall codex", "szr discover [--all|--since <n>|--json]"}, nil, ""},
		...
		{"discover empty", []string{"discover", "--root", filepath.Join(root, "no-transcripts")}, 0, []string{"no agent transcripts found"}, nil, ""},
		...
		{"json preview", []string{"json", "--mode", "preview", "--max-lines", "3", jsonFile}, 0, []string{"root: object keys=2", "a=\"x\"", "b: array len=1 sample=object{c}"}, nil, ""},
