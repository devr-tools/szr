...
func TestTeeList(t *testing.T) {
	app := newTeeAppWithEntries(t)
...
func TestTeeLatest(t *testing.T) {
	app := newTeeAppWithEntries(t)
...
func TestTeeReadByID(t *testing.T) {
	app := newTeeAppWithEntries(t)
...
func TestTeeEmptyList(t *testing.T) {
	app := newEmptyTeeApp(t)
	code, stdout, stderr := testutil.RunApp(t, app, "tee")
...
func TestTeeMissingLatest(t *testing.T) {
	app := newEmptyTeeApp(t)
	code, stdout, stderr := testutil.RunApp(t, app, "tee", "--latest")
...
func TestTeeMultipleIDs(t *testing.T) {
	app := newEmptyTeeApp(t)
...
func TestTeeMissingArtifactFile(t *testing.T) {
	root := t.TempDir()
...
func newEmptyTeeApp(t *testing.T) *cli.App {
	t.Helper()
	root := t.TempDir()
	paths := testutil.Paths(root)
	testutil.EnsurePaths(t, paths)
	return cli.NewWithDependencies("test", config.Default(), paths, history.New(paths.HistoryFile), testutil.AppEngine(t, paths))
}

func newTeeAppWithEntries(t *testing.T) *cli.App {
	t.Helper()
	root := t.TempDir()
	paths := testutil.Paths(root)
	testutil.EnsurePaths(t, paths)
